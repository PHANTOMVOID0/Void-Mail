...
// ── INBOX SESSION EXPIRY (5 min) ──────────────
const INBOX_TTL_MS       = 5 * 60 * 1000;
let inboxExpireTimer     = null;
let inboxCountdownTimer  = null;
let inboxExpiresAt       = 0;
...
// ── INBOX SESSION EXPIRY ─────────────────────

function clearInboxExpiry() {
  if (inboxExpireTimer !== null)    { clearTimeout(inboxExpireTimer);  inboxExpireTimer = null; }
  if (inboxCountdownTimer !== null) { clearInterval(inboxCountdownTimer); inboxCountdownTimer = null; }
  inboxExpiresAt = 0;
  updateInboxExpiry(null);
}

function updateInboxExpiry(msLeft) {
  const el = document.getElementById('inbox-ttl');
  if (!el) return;
  ...
  const s = totalSec % 60;
  el.textContent = `\u23f3 INBOX EXPIRES IN ${m}:${String(s).padStart(2, '0')}`;
  el.style.display = 'block';
}

function startInboxExpiry() {
  clearInboxExpiry();
  inboxExpiresAt = Date.now() + INBOX_TTL_MS;
  updateInboxExpiry(INBOX_TTL_MS);

  inboxCountdownTimer = setInterval(() => {
    const rem = inboxExpiresAt - Date.now();
    if (rem <= 0) { clearInterval(inboxCountdownTimer); inboxCountdownTimer = null; }
    else updateInboxExpiry(rem);
  }, 1000);

  inboxExpireTimer = setTimeout(() => {
    if (inboxCountdownTimer) { clearInterval(inboxCountdownTimer); inboxCountdownTimer = null; }
    inboxExpiresAt = 0;
    updateInboxExpiry(null);
    stopRefresh();
    inboxUnlocked = false;
    ...
    showToast('\u23f1 INBOX EXPIRED \u2014 GENERATE A NEW IDENTITY', 5000);
  }, INBOX_TTL_MS);
}
